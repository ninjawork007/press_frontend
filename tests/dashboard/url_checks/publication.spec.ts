...
test.describe("dashboard", () => {
    ...
    test('navigate to settings publications works', async ({ page }) => {
        const publications = page.getByText('Manage publications');
        await publications.click();
        await expect(page).toHaveURL(process.env.TEST_APP_BASE_URL, '/publications');
    })
})