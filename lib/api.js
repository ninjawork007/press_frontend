...
class Api {
  ...
  questionnaires = {
    findAll: async () => {
      ...
      return this.apiClient.get(`/questionnaires?${query}`);
    },
  };
  referrals = {
  ...
  purchasedPublications = {
    ...
    find: async ({
      ...
      if (session?.profile?.is_affiliate) {
        filters.ref_id = session?.profile?.id
      }
      if (site_id) {
        filters.site = {
          id: site_id
      ...
      return _this.apiClient.get(`/purchased-publication-list?${query}`);
    },
    getForProfile: async ({
      pageNumber = 1,
      status = null,
      ...
      const query = qs.stringify(
        {
          ...
          populate: {
            ...
            article: {
              populate: ["campaign", "drafts", "images"],
            },
          },
        },
        {
  ...
  campaigns = {
    ...
    find: async ({ pageNumber = 0, pageSize = 10, searchQuery = "" }) => {
      const session = await this.authorizeRequest();
      let filters = {};
      ...
      return this.apiClient.get(`/campaign-list?${query}`);
    },
